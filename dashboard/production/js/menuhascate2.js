...
function AddCateToMenu(){
  ...
  var lcategories = JSON.parse(_store_items);
  if(lcategories){
      var lreorder = lcategories.length;
  }
  ...
  for (var i = 0;i <= _e_array_check.length - 1; i++) {
    if(_e_array_check[i].checked){
        ...
        arr_item[t] = { idmenuhascate: 0, idmenu:_idmenu, idcategory:_idcategory, namemenu:_namemenu ,idparent:_idparent, reorder:lreorder, depth:0, trash:0 };  
        t = t + 1;
        lreorder = lreorder + 1;
    }
  }
 var _arr_item = JSON.stringify(arr_item);
               ...
               //_e_parent.style.marginBottom = "0px";
                ...
                if(pos_x > _left){
                    if(_e_prev){
                        ...
                        var _prev_idcategory = parseInt(_e_prev.getElementsByClassName("idmenuhascate")[0].value);
                        ...
                        for (var k = categories.length - 1; k >= 0; k--) {
                            var _id_menuhascate = parseInt(categories[k].idmenuhascate);
                            if(_id_menuhascate ==_idmenuhascate_curent){  
                                ...
                                if(cmp == 0){
                                    ...
                                    for (var t = k - 1; t >= 0; t--) {
                                        var _cmp_depth = parseInt(categories[t].depth);
                                        if(_cmp_depth==abs_cmp){
