   ...
   <div class="row">
	...
	<div class="card">
	    <div class="card-body">
			...
			@endif
			<div align="right">
				<a class="btn btn-default btn-primary" href="{{ route('admin.roles.create') }}">Thêm mới</a>
			</div>
	                ...
	                <tbody>
	                	@foreach($roles as $row)
							<tr>
								...
								<td class="btn-control"><a class="btn btn-primary btn-edit" href="{{ action('Admin\RoleController@edit',$row['idrole']) }}"><i class="fa fa-edit"></i></a></td>
								<td class="btn-control">
								     <form method="post" class="delete_form" action="{{action('Admin\RoleController@destroy', $row['idrole'])}}">
								      {{csrf_field()}}
